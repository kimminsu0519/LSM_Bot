...
def generate_launch_description():
    ...
    # Add resource paths for Gazebo Sim
    try:
    ...
    lsm_parent_dir = os.path.abspath(os.path.join(pkg_lsm_bot_description, '..'))

    current_gz_res = os.getenv('GZ_SIM_RESOURCE_PATH', '')
    res_dirs = [d for d in [tb3_parent_dir, lsm_parent_dir, current_gz_res] if d]
    new_gz_res = ":".join(res_dirs)
    ...
    # Load URDF for MiR200 AMR
    urdf_path = os.path.join(pkg_lsm_bot_description, 'urdf', 'mir200_amr.urdf')
    ...
    # Spawn MiR200 on Charging Pad (-68.0, -16.0, 0.2)
